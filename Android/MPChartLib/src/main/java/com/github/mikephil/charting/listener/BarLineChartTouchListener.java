...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<?
    ...
    private MPPointF mTouchNowPoint = MPPointF.getInstance(0, 0);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                } else if (mTouchMode == DRAG_SCALE) {
                    mChart.disableScroll();
                    performDragScale(event);
                }
                break;
    ...
    /**
     * Edited by MobileDev
     * Performs draw opertions for dragging on the chart.
     */
    private void performDragScale(MotionEvent event) {
        mLastGesture = ChartGesture.DRAG_SCALE;

        mTouchNowPoint.x = event.getX();
        mTouchNowPoint.y = event.getY();
        mChart.setSelection(mTouchStartPoint, mTouchNowPoint);
    }
