...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    // Editied by MobileDev
    private boolean mDragScaleEnabled = true;
    private float mSelectionStartX = 0f, mSelectionEndX = 0f;
    ...
    /**
     * Edited by MobileDev
     * paint object for drawing selection mask when selecting scale area.
     */
    ...
    protected void init() {
        ...
        mSelectionPaint.setColor(Color.argb(180, 0, 0, 0));
    }
     ...
     * Edited by MobileDev
     */
    protected void drawSelection(Canvas c) {
        if ((mSelectionEndX - mSelectionStartX) == 0)
            return;

        Transformer trans = getTransformer(AxisDependency.LEFT);
        float[] lineBuffers = new float[8];
        lineBuffers[0] = mSelectionStartX;
        lineBuffers[1] = getYChartMax();
        lineBuffers[2] = mSelectionEndX;
        lineBuffers[3] = getYChartMax();
        lineBuffers[4] = mSelectionEndX;
        lineBuffers[5] = getYChartMin();
        lineBuffers[6] = mSelectionStartX;
        lineBuffers[7] = getYChartMin();

        trans.pointValuesToPixel(lineBuffers);
        c.drawLines(lineBuffers, mSelectionPaint);
    }
    ...
    public void setSelection(MPPointF start, MPPointF end) {
        Entry entryStart = getEntryByTouchPoint(start.x, start.y);
        Entry entryEnd = getEntryByTouchPoint(end.x, end.y);

        float x1 = entryStart.getX();
        float x2 = entryEnd.getX();
        float tmp = x1;
        if (x1 > x2) {
            x1 = x2;
            x2 = tmp;
    ...
    public void setDragScaleEnabled(boolean enabled) { mDragScaleEnabled = enabled; }
